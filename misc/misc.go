...
//GetKhulnasoftEventMap returns the event structure from the client payload
func GetKhulnasoftEventMap(khulnasoftEvent interface{}) (map[string]interface{}, bool) {

	khulnasoftEventMap, ok := khulnasoftEvent.(map[string]interface{})
	if !ok {
		return nil, false
	}
	khulnasoftMsg, ok := khulnasoftEventMap["rl_message"]
	if !ok {
		return nil, false
	}
	khulnasoftMsgMap, ok := khulnasoftMsg.(map[string]interface{})
	if !ok {
		return nil, false
	}
	return khulnasoftMsgMap, true
}

//GetKhulnasoftEventVal returns the value corresponding to the key in the message structure
func GetKhulnasoftEventVal(key string, khulnasoftEvent interface{}) (interface{}, bool) {

	khulnasoftMsgMap, ok := GetKhulnasoftEventMap(khulnasoftEvent)
	if !ok {
		return nil, false
	}
	khulnasoftVal, ok := khulnasoftMsgMap[key]
	if !ok {
		return nil, false
	}
	return khulnasoftVal, true
}

//ParseKhulnasoftEventBatch looks for the batch structure inside event
func ParseKhulnasoftEventBatch(eventPayload json.RawMessage) ([]interface{}, bool) {
