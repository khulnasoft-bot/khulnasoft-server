...
import (
	...
	"github.com/khulnasoft/khulnasoft-server/gateway"
	"github.com/khulnasoft/khulnasoft-server/integrations"
	"github.com/khulnasoft/khulnasoft-server/jobsdb"
	"github.com/khulnasoft/khulnasoft-server/misc"
...
func (proc *HandleT) mainLoop() {
	...
	for {
		...
		for _, batchEvent := range combinedList {
			...
			eventList, ok := misc.ParseKhulnasoftEventBatch(batchEvent.EventPayload)
			if ok {
